#!/usr/bin/env python3
"""
Generate Excel output with KDJ signals and color-coded positions
"""
...
def run_kdj_excel_generation(data_file: str, output_file: str = None):
    """Run KDJ strategy and generate Excel with color-coded signals."""
    ...
    # Excel export is now handled by the reporter module
    
    # Save Excel output
    if output_file is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = f"output/excel_reports/kdj_signals_{timestamp}.xlsx"
    
    print("🎨 Creating Excel file with color coding...")
    output_path = strategy.save_excel_output(output_file)
    ...
    print(f"\n🎨 Color Legend:")
    for status, description in info['color_coding'].items():
        print(f"   {status:5s}: {description}")
    
    print(f"\n📋 Excel contains:")
    print(f"   - Main sheet: KDJ Strategy Signals (with colors)")
    print(f"   - Legend sheet: Color explanations and parameters")
...
if __name__ == "__main__":
    ...
    # Generate Excel
    try:
        output_path = run_kdj_excel_generation(data_file)
        print(f"\n🎉 Success! Open the Excel file: {output_path}")
        print("💡 Each position type has its own color for easy visual analysis!")
