#!/usr/bin/env python3
"""
Modified backtest for MACD+RSI strategy with RSI thresholds 80/20 instead of 70/30.
"""
...
def generate_modified_signals(df):
    """Generate trading signals with RSI thresholds 80/20."""
    
    print(f"🔄 Generating signals with RSI thresholds 80/20...")
    ...
    for idx in range(1, len(ready_bars)):  # Start from 1 to have previous values
        ...
        if current_position is not None:
            ...
            if current_position == 'LONG':
                ...
                # Take profit at +0.8% or stop loss at -0.6%
                if pnl_pct >= 0.8 or pnl_pct <= -0.6:
        ...
        # Entry signals (only when no position)
            ...
            if (macd_bullish_cross and 
                rsi < 80 and  # Changed from 70 to 80
                current_price > ema50):
            ...
            # SHORT: MACD bearish cross + RSI > 20 + Price < EMA50
                  ...
                  rsi > 20 and  # Changed from 30 to 20
                  current_price < ema50):
...
def run_backtest_with_modified_signals(df):
    """Run backtest with modified RSI signals."""
    
    print(f"\n🚀 Starting backtest with RSI 80/20 thresholds...")
...
def display_comparison(results, metrics, initial_capital):
    """Display results with comparison to original."""
    
    print(f"\n" + "="*80)
    print(f"📊 BACKTEST RESULTS - MACD+RSI Strategy (RSI 80/20)")
    print(f"="*80)
    ...
    print(f"\n🔄 COMPARISON WITH RSI 70/30:")
    print(f"   Previous Return: +1.20%  →  Current Return: {metrics['total_return_pct']:+.2f}%")
    print(f"   Previous Trades: 251     →  Current Trades: {results['total_trades']}")
    print(f"   Previous Win Rate: 47.01% →  Current Win Rate: {metrics['win_rate_pct']:.2f}%")

def save_results(results, metrics, initial_capital):
    """Save results with RSI 80/20 suffix."""
    
    # Save trades to CSV
    if len(results['trades']) > 0:
        trades_df = pd.DataFrame(results['trades'])
        trades_file = '/Users/alexey/Documents/Development/Python/Trading_bot/output/backtest_3months_trades_rsi_80_20.csv'
        trades_df.to_csv(trades_file, index=False)
        print(f"\n💾 Trades saved to: {trades_file}")
    
    # Save summary
    summary = {
        'strategy_name': 'MACD+RSI RSI80/20',
    ...
    summary_file = '/Users/alexey/Documents/Development/Python/Trading_bot/output/backtest_3months_summary_rsi_80_20.csv'
...
def main():
    ...
    print("🚀 Starting Modified MACD+RSI Backtest with RSI 80/20")
    print("Strategy: MACD+RSI with RSI thresholds 80/20 (was 70/30)")
    
    # Load data
    df = load_data()
    
    # Generate modified signals with RSI 80/20
    ...
    print(f"📊 RSI thresholds changed from 70/30 to 80/20")
    print(f"🔍 Impact: {'More' if results['total_trades'] > 251 else 'Fewer'} trades, {'Better' if metrics['total_return_pct'] > 1.20 else 'Worse'} performance")
